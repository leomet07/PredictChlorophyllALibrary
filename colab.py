# -*- coding: utf-8 -*-
"""ExportSatelliteRaster.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1JpI6bmyCTl7r5OHJM9RT32elcp-hUPpW
...
try:
        ...
        ee.Authenticate()
        ee.Initialize(project='leomet07-nasa-lakes-ccri')
...
#assets from GEE
LakeShp = ee.FeatureCollection("users/greeneji/LAGOS_NE_All_Lakes_4ha")
#Choose which lake by lake ID
LakeShp = ee.FeatureCollection(LakeShp.filter(ee.Filter.eq('lagoslakei', 16364)))


# lagoslakei = pd.read_csv(io.BytesIO(uploaded['allNY_lakes_4ha.csv']))
...
sum23 = ee.Filter.date('2019-08-01','2019-08-31')
...
# inspect test
num_bands = test.getInfo()
print(num_bands)

"""Make sure to check for homogenous band dimensions before dimensions are set for export."""

# clip test to shapefile
test = test.clip(LakeShp)
# get test dimensions
info = test.getInfo()
bands = info['bands']
width = -1
...
test = test.clipToBoundsAndScale(geometry=LakeShp, width=width, height=height)

# out_dir = os.path.expanduser('Export_Rasters_071')
# if not os.path.exists(out_dir):
#     os.makedirs(out_dir)

import time

task = ee.batch.Export.image.toDrive(image=test.clip(LakeShp),
                                     fileFormat='GeoTIFF',
                                     description='test_143pm',
                                     folder='Export_Rasters_0716',
                                     maxPixels=1e13)
