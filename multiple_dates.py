...
if __name__ == "__main__":
    ...
    date_range = list(pd.date_range(start=start_date_range, end=end_date_range, freq=f"{frequency}D"))
    ...
    for i in range(len(date_range) - 1): # stop iterating one element short
        ...
        try:
            export_raster_main(out_dir, out_filename, project, lakeid, start_date, end_date, scale)
