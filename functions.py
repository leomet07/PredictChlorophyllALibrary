...
def get_masked_coll(LakeShp, start_date, end_date):
    img = get_s2_sr_cld_col(start_date, end_date, LakeShp).map(add_cld_shdw_mask).map(apply_cld_shdw_mask)
    return img
...
def import_collections(masked_coll, range, LakeShp) -> ee.Image:
    ...
    FC_S2A = masked_coll.filter(range) \
    ...
    FC_S2B = masked_coll.filter(range) \
                    .filterBounds(LakeShp) \
    ...
    first_image_date_S2B = ee.Date(finger_lakes.get('system:time_start')).format('YYYY-MM-dd').getInfo()
...
def inspect_raster(image):
    ...
    if (num_bands != 9):
        ...
        print("Image has 9 bands, good to go ahead and add 4 more constant bands and run model.")
        info = image.getInfo()
        pprint(info)
        bands = info['bands']
        width = -1
        ...
        for band in bands:
            ...
            print(f"Band: {band['id']}, Width: {dimensions[0]} pixels, Height: {dimensions[1]} pixels")
            width = dimensions[0]
            height = dimensions[1]
...
def export_raster_main(out_dir: str, out_filename: str, project: str, lakeid: int, start_date, end_date):
    # open gee project
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print("python export_raster.py <out_dir> <project> <lakeid> <start_date> <end_date> <out_filename>")
        sys.exit(1)
    ...
    export_raster_main(out_dir=out_dir, out_filename=out_filename, project=project, lakeid=lakeid, start_date=start_date, end_date=end_date)
